...
class AQIDataPreprocessor:
    ...
    def remove_outliers(self, df, columns=None, method="iqr", threshold=1.5):
        ...
        print(
            f"✅ Outliers removed: {removed_rows} rows ({removed_rows / initial_rows * 100:.2f}%)"
        )
    ...
    def create_features(self, df):
        ...
        if "Traffic_Volume" in df.columns and "Industrial_Activity" in df.columns:
            df_features["Human_Activity_Index"] = (
                ...
                + df_features["Industrial_Activity"]
            ) / 2
        ...
        if "PM2.5" in df.columns and "PM10" in df.columns:
            df_features["PM2.5_to_PM10_Ratio"] = (
                df_features["PM2.5"]
                / (df_features["PM10"] + 1)  # +1 to avoid division by zero
            )
    ...
    def get_preprocessing_summary(self, df_original, df_processed):
        ...
        print(
            f"Missing Values: {summary['original_missing']} → {summary['processed_missing']}"
        )
...
if __name__ == "__main__":
    ...
    print(
        "  X_train, X_test, y_train, y_test, scaler, features = preprocess_pipeline('data.csv')"
    )
